...
def get_types_id_array(types_array):
    return FakeAiidaT.get_types_id_array(types_array)


# aiida
def get_analisi_traj_from_aiida(traj, tskip=1):
    t = FakeAiidaT(traj, traj_skip=tskip)
    atraj = t.get_analisi_traj(wrapped=True)
    atraj_unw = t.get_analisi_traj(wrapped=False)
...
def analyze_vdos_numpy(pos, vel, types, box,
    ...
    traj = pa.Trajectory(pos[start:start + nstep, :, :], vel[start:nstep + start, :, :], types,
                         box[start:start + nstep, :, :],
...
def analyze_sh(traj, start, stop, startr, endr, nbin, ntypes=2, tmax=0, nthreads=4, tskip=10, print=print,
    ...
    sh = pyanalisi_wrapper('SphericalCorrelations', traj
                           ...
                           , True,
                           sann)  # flag that at the moment does nothing, sann if it is not empty activates the sann algorithm.
...
def plot_msd(times, res, cm, title='', res_var=None, fig_ax=None, subplots_kw=DEFAULT_SUBPLOTS_KW):
    ...
    for i in range(res.shape[-1]):
        plt_err(ax, times[:res.shape[0]] - times[0], res[:, cm, i],
                res_var[:, cm, i] if res_var is not None else res_var, label='type=' + str(i))
    ax.legend()
...
def fit_sh(times, res, type1, type2, ibin, lmin=0, lmax=11,
    ...
    for l in range(lmin, lmax):
        ...
        if pre_fit > 0:
            ...
            p0, cv = fit_double_exp(t[idx_fit_beg:idx_fit_0],
                                    r[idx_fit_beg:idx_fit_0, type1, type2, ibin, i] / r[0, type1, type2, ibin, i])
            kwargs['p0'] = p0
        p, cv = fit_double_exp(t[idx_fit_beg:idx_fit],
                               r[idx_fit_beg:idx_fit, type1, type2, ibin, i] / r[0, type1, type2, ibin, i], **kwargs)
        if abs(1.0 - math.exp(-p[-2] * t[
            idx_fit - 1])) < 1e-5:  # safely assume that the last exponential is approximable by a constant
            p = norm_fit(p)
        fits.append(p)
    return fits
    ...
    for l in range(lmin, lmax):
        ...
        def plot(ax):
            ...
            plt_err(ax, t, r[:, type1, type2, ibin, i] / r[0, type1, type2, ibin, i],
                    (r_var[:, type1, type2, ibin, i] / r[
                        0, type1, type2, ibin, i] ** 2) if r_var is not None else r_var, label='l=' + str(l))
...
def atomic_density(traj, dr=0.1, start=-1):
    ...
    print(box)
    hist = pyanalisi_wrapper('PositionHistogram', atraj, np.array(2 * np.rint(box[3:6] / (2 * dr)), dtype=int).tolist(),
                             1, 1)
...
def density_field(res, box, box_kw={}, plot=None, sph_max_idx=0, gaussian_filter=False, gaussian_filter_radius=1.2):
    ...
    def button1_on_click(b):
        ...
        idx, (_z, _y, _x) = get_max_in_spherical_domain(res[sph_max_idx], _w_x, _z, _y, _x,
                                                        l=(2 * box[5], 2 * box[4], 2 * box[3]))
        # _x,_y,_z=-_x,-_y,-_z
    ...
    wlist.append(interactive(g5, x=widgets.FloatSlider(value=box[0] + box[3], min=box[0], max=box[0] + 2 * box[3],
                                                       step=box[3] / 100, layout=box_layout)))
    ...
    wlist.append(interactive(g6, w_x=widgets.FloatSlider(value=box[3] / 5, min=0.0, max=2 * box[3], step=box[3] / 100,
                                                         layout=box_layout)))
    ...
    wlist.append(interactive(g7, y=widgets.FloatSlider(value=box[1] + box[4], min=box[1], max=box[1] + 2 * box[4],
                                                       step=box[4] / 100, layout=box_layout)))
    ...
    wlist.append(interactive(g8, w_y=widgets.FloatSlider(value=2 * box[4], min=0, max=2 * box[4], step=box[4] / 100,
                                                         layout=box_layout)))
    ...
    wlist.append(interactive(g9, z=widgets.FloatSlider(value=box[2] + box[5], min=box[2], max=box[2] + 2 * box[5],
                                                       step=box[5] / 100, layout=box_layout)))
    ...
    wlist.append(interactive(g10, w_z=widgets.FloatSlider(value=2 * box[5], min=0.0, max=2 * box[5], step=box[5] / 100,
                                                          layout=box_layout)))
...
def force_ratio_histogram(wf, print=print, ax=[], create_fig=lambda: plt.subplots(**DEFAULT_SUBPLOTS_KW)):
    pwcalcjobs = []
    for c in [x for x in wf.called if
              str(x.process_class) == "<class 'aiida_quantumespresso.calculations.pw.PwCalculation'>"]:
        pwcalcjobs.append(c)
        print(c.pk)
...
def print_cp_with_traj(wf, print=print, min_t=0.0):
    ...
    for c in l:
        print("===========")
        print(c.pk, 'traj pk={}: {:.3}ps'.format(c.outputs.output_trajectory.pk, length_traj(
            c.outputs.output_trajectory)) if 'output_trajectory' in c.outputs else 'No output_trajectory')
...
def analisi2aiida_traj(t, symbols):
    ...
    return ft.get_aiida_traj()


def read_lammps_bin(f, **kwargs):
    '''
    ...
    '''
    return FakeAiidaT.from_lammps_binary(f,**kwargs)
...
def get_type_mask_from_s(symbols):
    return FakeAiidaT.get_type_mask_from_s(symbols)


def gen_poscar(at, every=1, start=0, desc=''):
    traj = FakeAiidaT(at)
    return traj.get_poscar(every=every, start=start, desc=desc)
...
def wrap_dataset(tt):
    ...
    wrapped = FakeAiidaT({
        'positions': tt['positions'].reshape(nts, nat, 3),
        'forces': tt['forces'].reshape(nts, nat, 3),
        'cells': tt['cells'].reshape(nts, 3, 3)
    })
    wrapped.rotate_qr()
    return {'cells': wrapped.get_array('cells').reshape((nts, 9)),
            'cells_unrotated': tt['cells'],
            'positions': wrapped.get_array('positions').reshape((nts, nat * 3)),
            'forces': wrapped.get_array('forces').reshape((nts, nat * 3)),
            'energy': tt['energy'],
            'stress': wrapped.get_array('stresses').reshape((nts,9))
            }
...
def show_atraj(tr, atraj, wrap=True, plot=None, fast=1.0):
    ...
    for sp, mask in zip(atomic_species, masks):
        plot += k3d.points(positions={str(t / 30.0 / fast): pos_m[t, mask, :] for t in range(pos_m.shape[0])}, size=1.0,
                           name=sp)
        print(sp)
    return plot
...
def gofr_movie(atraj, skip=5, n_segments=10, gofr_kw={}, plt_gofr_kw={}, callax=lambda x: None):
    ...
    class SteinAni:
        ...
        def __call__(self, i):
            self.axs.clear()
            self.fig, self.axs, _, _, _, _, _, _, _, _, _, _ = do_plots_gr_sh(*self.tstein[i], **self.plt_gofr_kw,
                                                                              fig_ax=(self.fig, self.axs))
...
def elastic_c(kbT, box_matrix):
    ...
    if is_diagonal:
        pick = pick[:3]
    CS_minus1 = np.einsum('ti,tk->ik', (strain - strain_0).reshape((nt, 9))[:, pick],
                          (strain - strain_0).reshape((nt, 9))[:, pick]) * vol_mean / kbT / nt * 1e9
...
def inspect(traj, only_cell=False, plot_traj=True, plot=True,
    ...
    print(
        f'close packed ideal structure delta = {hcpx - 2 * cell_transition_ave[3]} {hcpy - 2 * cell_transition_ave[4]} {hcpz - 2 * cell_transition_ave[5]}')
    ...
    if (cells_transition != cells_transition[0]).any():
        ...
        try:
            uma = 1.66e-27  # kg
            mcell = (sum([1 for i in traj.get_attribute('symbols') if i == 'H']) + sum(
                [14 for i in traj.get_attribute('symbols') if i == 'N'])) * uma
...
def print_all(all_res, prefix='', subplots_kw=DEFAULT_SUBPLOTS_KW):
    ...
    alist = [('NN peak r', grp_pos[:, 1]), ('NN peak width', grp_width[:, 1]), ('msd0', MSDs[:, 0]),
             ('msd1', MSDs[:, 1])] + ([('Vs', Vss)] if len(Vss) > 0 else [])
    try:
        for k, arr in alist:
        ...
        def plt_el(Ts, CS, density):
            ...
            for arr, label in [((CS[:, 0, 0] / density) ** .5, 'longitudinal'),
                               ((CS[:, 3, 3] / density) ** .5, 'shear')]:
                for i in range(len(Ts)):
                    ax.annotate(f'dt={DTs[i]:.3f}', xy=(Ts[i], arr[i]),
                                xytext=(-1, 1), textcoords="offset points",
                                horizontalalignment="right")
