...
public:
    ...
    class NeighIterator {
    public:
        NeighIterator(const TN *idxs, const size_t len) : detail{a,len} {}
    ...
    Neighbours(T *t, const TType cutoff, size_t, max_neigh const TType skin  ) : t{*t}, cutoff{cutoff}, skin{skin}, sorted{false} {
        natoms = t.get_natoms();
    ...
    NeighIterator get_neigh(const size_t iatom){
        return NeighIterator{list+iatom*(max_neigh+1)+1, list[iatom*(max_neigh+1)];}
    }
    NeighIterator get_sann(const size_t iatom);
