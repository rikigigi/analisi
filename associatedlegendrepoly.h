...
void chebyshevRecursiveAngleMultiplication(const T &x,const T &y, T * res) {
...
struct MultiVal {
    T val[l+1]; // only positive m are considered
    T val_minus[l]; //negative values of m
    MultiVal<l-1,T> val_;

    template <typename ...Args>
    void apply_f ( T f(int,int,T,Args...) , Args...args) {
        for (int m=0;m<l+1;++m) {
    ...
    template<int j>
    MultiVal<j,T> & valm() {if constexpr (j==l) return *this; else return val_.template valm<j>();}
};
...
template <int lmax,int l, int m, class T  = double>
struct AssociatedLegendrePoly
{
    static void calc1(const T &x, MultiVal<lmax,T> & res) {
        /* l+1,l step:
        ...
        */
        ...
        if constexpr (l<lmax){
            AssociatedLegendrePoly<lmax,l+1,m,T>::calc2(x,res); //start l+1,m recursion
        }
    }
    static void calc2(const  T &x, MultiVal<lmax,T> & res){
        /*
        ...
        */
        res.template valm<l>().val[m]=(res.template valm<l-1>().val[m] *x*(2*l-1) - res.template valm<l-2>().val[m] *(l+m-1))/
                                                          (l-m)                                    ;
        if constexpr(l<lmax) {
            AssociatedLegendrePoly<lmax,l+1,m,T>::calc2(x,res); //continue l+1,m recursion
        }

    }
};

template <int lmax, int l,class T>
struct AssociatedLegendrePoly<lmax,l,l,T> {
    static void calc (const T&x,  MultiVal<lmax,T> & res) {
        /* l,l recursion:
         * Implements the following:
        ...
        */
        ...
        if constexpr (lmax>l){
            AssociatedLegendrePoly<lmax,l+1,l+1>::calc(x,res);//continue l,l recursion
            AssociatedLegendrePoly<lmax,l+1,l>::calc1(x,res);//start a new l+1,l recursion
        }
    }
};

template <int lmax, class T>
struct AssociatedLegendrePoly<lmax,0,0,T> {
   static void calc (const T&x,  MultiVal<lmax,T> & res) {
        //begin with l=0 and go up till lmax
        //store the result while recursing in res
        res.template valm<0>().val[0]=1.0;
        if constexpr (lmax>0){
            AssociatedLegendrePoly<lmax,1,1,T>::calc(x,res);//start l,l recursion
            AssociatedLegendrePoly<lmax,1,0,T>::calc1(x,res);//start l+1,l recursion
        }
    }
};
